...
! ***********************************************************************
      ...
      subroutine do_create_zams( &
         ...
         mass_loop: do i=1, n
            ...
            call star_create_pre_ms_model( &
               ...
               s% job% change_initial_net, s% job% new_net_name, &
         ...
         subroutine write_index_head
            ...
            write(io_ms_index, 4) 'net_name', "'"// trim(s% job% new_net_name) // "'"
      ...
      integer function evolve_to_zams_check_model(s, id, lipar, ipar, lrpar, rpar)
         ...
         !if (s% L_nuc_burn_total >= s% L(1)/Lsun) then !default zams
         !termination
         !if (((initial_center_h1 - s% center_h1) / initial_center_h1 > 0.001) .and. (Lnuc_fraction > 0.99)) then
         if (Lnuc_fraction > 0.99) then
      ...
      !! down below from the standard posydon run_star_extras
