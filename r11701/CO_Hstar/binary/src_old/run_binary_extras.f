...
! ***********************************************************************
      ...
      integer function extras_binary_check_model(binary_id)
       ...
       ! Calculate radiative efficiency eta for accretor for compact accretor !!!modified here
       if (b% m(2)/Msun < 2.50) then
           if (0.17 <= b% m(2)/Msun .and. b% m(2)/Msun <= 1.25) then ! Radius for WD: if supported by non-relativistic electron degeneracy pressure
           ...
           b% mdot_edd_eta = b% s_donor% cgrav(1) * b% m(2) / (clight ** 2 * acc_radius) ! Radiative efficiency eta for accretor
           b% mdot_edd = 4d0*pi*b% s_donor% cgrav(1)*b% m(b% a_i)&
                              /(clight*0.2d0*(1d0+b% s_donor% surface_h1)*b% mdot_edd_eta)
           b% mdot_edd_accretion_luminosity = b% mdot_edd_eta*b% xfer_fraction*abs(b% mtransfer_rate)*clight**2
       end if
      !write(*,'(g0)') b% mdot_edd_accretion_luminosity
       ...
       if ((b% rl_relative_gap(1) .ge. 0.d0) .or. (-b% mtransfer_rate .ge. 1d-10 ) ) then
          ...
          b% s1% Dutch_scaling_factor = 1.d0
          ! Binary evolution
