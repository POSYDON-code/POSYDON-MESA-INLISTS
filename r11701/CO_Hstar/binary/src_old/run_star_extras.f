...
contains
  ...
  integer function extras_startup(id, restart, ierr)
    ...
    s% RGB_to_AGB_wind_switch = 1d-4
    s% cool_wind_full_on_T = 0.8d4
    s% hot_wind_full_on_T = 1.2d4
    s% hot_wind_scheme = 'Dutch'
    s% Dutch_scaling_factor = 1.0d0
    if(s% mstar < 10.0d0)then !not using the other_wind from MIST
      s% cool_wind_RGB_scheme = 'Reimers'
      s% cool_wind_AGB_scheme = 'Blocker'
      s% Reimers_scaling_factor = 0.1 !0.5d0 !0.1 MIST
      s% Blocker_scaling_factor = 0.2 !0.1d0 !0.2 MIST
      !if(s% surface_h1 > 0.4) s% Dutch_scaling_factor = 0.0d0 (MESA will idetntify them itself and go to eval_Vink_wind instead of Dutch)
    else
