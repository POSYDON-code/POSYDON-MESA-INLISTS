   ...
   !1                /(s% omega_avg_surf/(2d0*pi)), f_conv
      ...
      subroutine my_mdot_edd(binary_id, mdot_edd, ierr)
	     ...
	     mdot_edd_old = 4d0*pi*b% s_donor% cgrav(1)*b% m(b% a_i) &
             ...
             mdot_edd_limit = abs(10d0*mdot_edd_old) * 0.367d0 * pow_cr(abs(10d0), -0.11d0)

	     mdot_edd = abs(b% mtransfer_rate) * 0.367d0 * pow_cr(abs(b% mtransfer_rate)/mdot_edd_old, -0.11d0)
