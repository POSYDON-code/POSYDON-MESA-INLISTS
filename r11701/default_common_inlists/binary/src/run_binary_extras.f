...
! ***********************************************************************
      ...
      subroutine extras_binary_controls(binary_id, ierr)
         ...
         ! to disable the printed warning message,
          ...
          b% other_check_implicit_mdot => my_check_implicit_mdot
   ...
   !1                /(s% omega_avg_surf/(2d0*pi)), f_conv
	 ...
	 real(dp) :: r_l2, d_l2
         ...
         !i think this technically does nothing because binary_evolve starts CE when mtransfer_rate == max_implicit_abs_mdot
          if (abs(b% mtransfer_rate/(Msun/secyer)) >= 1d-1) then      
            ...
            if (.not. b% CE_flag) then ! turn CE only once
             ...
             i_acc = 2
               if (b% m(i_don) .gt. b% m(i_acc)) then !mdon>macc, q<1
                  ...
                  if (b% r(i_don) .ge. (r_l2)) then
                     ...
                     if (.not. b% CE_flag) then ! turn CE only once
                        write(*,'(g0)') "reached regime overflow from L2 (R_L2) surface, begin CE ..."
                        b% CE_flag = .true.
                     end if 
	 ...
	 if (b% point_mass_i == 0) then
      ...
      integer function my_check_implicit_rlo(b, new_mdot)
         type (binary_info), pointer :: b
         ...
         if (b% mdot_scheme == "roche_lobe") then
            ...
            if (b% CE_flag) then ! added to check for CE_flag and set correct new_mdot
      ...
      end function my_check_implicit_rlo
