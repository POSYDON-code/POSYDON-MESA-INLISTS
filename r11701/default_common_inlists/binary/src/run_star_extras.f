...
!     ***********************************************************************
      ...
      subroutine data_for_extra_history_header_items(id, &
                                                     id_extra, num_extra_header_items, &
                                                     extra_header_item_names, extra_header_item_vals, &
                                                     ierr)
      ...
      subroutine data_for_extra_history_columns(id, id_extra, n, names, vals, ierr)
         ...
         do k=1,s% nz - 1
            !envelope is defined to be H-rich
            if (s% m(k) > (s% he_core_mass * Msun)) then 
         ...
         call loop_conv_layers(s,n_conv_regions_posydon, n_zones_of_region, bot_bdy, top_bdy, &
                               cz_bot_mass_posydon, cz_bot_radius_posydon, &
                               cz_top_mass_posydon, cz_top_radius_posydon)
         if (n_conv_regions_posydon > 0) then
            do k=1, n_conv_regions_posydon ! from inside out
               ! if the conv. region is not inside the conv. core
               if ((cz_bot_mass_posydon(k) / Msun) >=  m_conv_core) then 
                  m_env_new = (cz_top_mass_posydon(k) - cz_bot_mass_posydon(k)) / Msun
                  ! depth of the convective layer, length of the eddie
                  Dr_env_new = cz_top_radius_posydon(k) - cz_bot_radius_posydon(k) 
      ...
      real(dp) function get_ion_info(s,id,k)
         ...
         type (star_info), pointer :: s
         ierr = 0
         call eval_ionization(1d0 - (s% X(k) + s% Y(k)), &
                              s% X(k), s% Rho(k), s% lnd(k)/ln10, &
      ...
      subroutine set_core_info(s, k, core_k, core_m, core_r)
         type (star_info), pointer :: s
         integer, intent(in) :: k
         integer, intent(out) :: core_k
         real(dp), intent(out) :: core_m, core_r
         integer :: j, jm1, j00
         ...
         call get_info_at_q(s, core_q, core_k, core_m, core_r)

      end subroutine set_core_info


      subroutine get_info_at_q(s, bdy_q, kbdy, bdy_m, bdy_r)

         type (star_info), pointer :: s
         real(dp), intent(in) :: bdy_q
         integer, intent(out) :: kbdy
         real(dp), intent(out) :: bdy_m, bdy_r
         ...
         bdy_r = pow_cr(interp2(s% r(k)*s% r(k)*s% r(k), &
                        s% r(k+1)*s% r(k+1)*s% r(k+1)),1d0/3d0)/Rsun
      ...
      integer function extras_finish_step(id, id_extra)
         ...
         ! this should only up for the 2 step of He star formation
         ...
         ! TP-AGB
         if (TP_AGB_check .and. (s% he_core_mass - s% c_core_mass < 1d-1) &
            .and. (s% center_he4 < 1d-6)) then
      ...
      subroutine extras_after_evolve(id, id_extra, ierr)
         ...
         call star_ptr(id, s, ierr)
         if (ierr /= 0) return
         !check for central carbon depletion, only in case we run single stars.
         if(s% x_logical_ctrl(1)) then 
            call star_write_profile_info(id,"LOGS1/final_profile.data",id,ierr)
         endif
      ...
      subroutine matt_torque(id, ierr)
         integer, intent(in) :: id
         integer, intent(out) :: ierr
         type (star_info), pointer :: s
      ...
      subroutine garraffo_torque(id, ierr)
         integer, intent(in) :: id
         integer, intent(out) :: ierr
         type (star_info), pointer :: s
         integer :: j, k, nz, i
      ...
      ! but modified to operate on single stars.
      subroutine carb_torque(id, ierr)
         integer, intent(in) :: id
         integer, intent(out) :: ierr
         integer :: i, k, nz
         type (star_info), pointer :: s
         real(dp) :: tau_convective
         real(dp) :: rsun4, two_pi_div_p2, K2
      ...
      subroutine rvj_torque(id, ierr)
         ...
         nz = s% nz
         i = s% n_conv_regions
         magnetic_braking_gamma = 4d0
         MOI = dot_product(s% dm_bar(1:s% nz), s% i_rot(1:s% nz))
         mstar = s% star_mass * msun
         rsurf = rsun * pow_cr(10d0, s% log_surface_radius)
         rstar_div_rsun = pow_cr(10d0, s% log_surface_radius)
      ...
      subroutine loop_conv_layers(s,n_conv_regions_posydon, n_zones_of_region, bot_bdy, top_bdy, &
                                  cz_bot_mass_posydon, cz_bot_radius_posydon, cz_top_mass_posydon, &
                                  cz_top_radius_posydon)
         type (star_info), pointer :: s
         ! integer, intent(out) :: ierr
            ...
            if (in_convective_region) then
               if (s% mixing_type(k) /= convective_mixing) then ! top of convective region
                  ...
                  if (pot_n_zones_of_region >= min_zones_for_convective_tides) then
                     ...
                     cz_top_mass_posydon(n_conv_regions_posydon) = s% M_center + &
                                                                   (s% q(k) - s% cz_bdy_dq(k))*s% xmstar
      ...
      subroutine other_set_mdot(id, L_phot, M_phot, R_phot, T_phot, wind, ierr)
         ...
         if(T1 <= s% hot_wind_full_on_T)then
            ...
            if ((s% he_core_mass - s% c_core_mass < 1d-1) .and. (s% center_he4 < 1d-6)) then
               ...
               if (dbg) write(*,1) 'using cool_wind_AGB_scheme: "' // trim(scheme) // '"', &
